...
int
machine_at_6alx_init(const machine_t *model)
{
    int ret;

    ret = bios_load_linear("roms/machines/6alx/9jj8-a.bin",
                           0x000e0000, 131072, 0);
    ...
    device_add(&gl518sm_2c_device);    /* fans: CPU, Fan 2, Chassis; temperatures: unused, CPU, unused */
...
machine_at_p6bxt_init(const machine_t *model)
{
    ...
    if (sound_card_current[0] == SOUND_INTERNAL)
        device_add(&cmi8738_device);

    return ret;
}

int
machine_at_p6zxt_init(const machine_t *model)
{
    int ret;

    ret = bios_load_linear("roms/machines/p6zxt/Zxt500.bin",
                           0x000c0000, 262144, 0);
...
int
machine_at_3isat_init(const machine_t *model)
{
    int ret;

    ret = bios_load_linear("roms/machines/3isat/3isat120.bin",
                           0x000c0000, 262144, 0);
