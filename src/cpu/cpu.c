...
int cpu_uncapped = 0; /* 1 = Bentium5 uncapped mode: no catch-up in the outer loop */
...
cpu_set(void)
{
    ...
    /* Uncapped mode: Bentium5 runs as fast as the host can; the outer scheduler
       loop will not try to catch up missed quanta so the VM degrades smoothly
       instead of stuttering when the host is slow. */
    cpu_uncapped = (cpu_s->cpu_type == CPU_BENTIUM5) ||
                   (cpu_s->cpu_type == CPU_PENTIUM) ||
                   (cpu_s->cpu_type == CPU_PENTIUMMMX) ||
                   (cpu_s->cpu_type == CPU_WINCHIP) ||
                   (cpu_s->cpu_type == CPU_WINCHIP2) ||
                   (cpu_s->cpu_type == CPU_K5) ||
                   (cpu_s->cpu_type == CPU_K6) ||
                   (cpu_s->cpu_type == CPU_K6_2) ||
                   (cpu_s->cpu_type == CPU_K6_2C) ||
                   (cpu_s->cpu_type == CPU_K6_2P) ||
                   (cpu_s->cpu_type == CPU_K6_3) ||
                   (cpu_s->cpu_type == CPU_K6_3P) ||
                   (cpu_s->cpu_type == CPU_Cx6x86) ||
                   (cpu_s->cpu_type == CPU_Cx6x86L) ||
                   (cpu_s->cpu_type == CPU_Cx6x86MX) ||
                   (cpu_s->cpu_type == CPU_PENTIUMPRO) ||
                   (cpu_s->cpu_type == CPU_PENTIUM2D) ||
                   (cpu_s->cpu_type == CPU_PENTIUM2) ||
                   (cpu_s->cpu_type == CPU_CYRIX3S);
...
#ifdef USE_DYNAREC
            if (cpu_s->cpu_type == CPU_BENTIUM5)
                codegen_timing_set(&codegen_timing_bentium5);
            else
